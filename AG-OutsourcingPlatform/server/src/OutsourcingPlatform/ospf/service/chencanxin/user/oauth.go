...
用户微信授权登陆的方法
*/
func (u *User) Oauth(w http.ResponseWriter, r *http.Request) error {
	if u == nil {
		return ErrOfNilUser
	}
	wc := u.Wechat
	...
	if auth.Context.Code == "" {
		auth.Redirect(w, r, "http://"+u.GetOpts().ClientDomain+constant.CONST_WECHAT_LOGIN_URL, "snsapi_userinfo",
			u.GetOpts().AppID, "ospf")
		return nil
	}
